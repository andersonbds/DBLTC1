...
from config import TOKEN, ADMIN_GROUP_ID, CANAL_LINK, GRUPO_LINK, GH_S_TO_REAL, MIN_SAQUE_PIX, MIN_SAQUE_LTC
...
def comprar_handler(message):
    ...
    try:
        ...
        if sucesso:
            notify_admin(f"Usuário {user_id} comprou personagem #{idx+1}")
    except Exception:
...
def resgatar_handler(message):
    ...
    bot.send_message(user_id, msg)
    if sucesso:
        notify_admin(f"Usuário {user_id} resgatou mineração do personagem {nome_pers}")
...
def indicacao_handler(message):
    ...
    texto = f"Seu link de indicação:\n{link}\n\n" \
            f"Ganhe até 1% de cashback nas compras da sua rede!"
    bot.send_message(user_id, texto)
